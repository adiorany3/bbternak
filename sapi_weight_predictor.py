...
# Helper function untuk membuat visualisasi simpel ternak
def create_animal_silhouette(animal_type, chest_size, body_length, gender="Jantan"):
    """
    Membuat visualisasi sederhana siluet ternak berdasarkan jenis dan ukuran.
...
if st.sidebar.button("Hitung Berat Badan", type="primary"):
    ...
    animal_image = create_animal_silhouette(jenis_ternak, lingkar_dada, panjang_badan, jenis_kelamin)
    st.image(animal_image, caption=f"Visualisasi {jenis_ternak} - {bangsa_ternak} ({jenis_kelamin})", use_column_width=True)
